...
def read_hyphen_list():
    ...
    :return
    """
    df = pd.read_csv(PATH_TO_RESOURCES.joinpath('hyphen-deletion.csv').as_posix())
    insert = df['insertion'].apply(lambda x: x.lower()).values.tolist()
    delete = df['deletion'].apply(lambda x: x.lower()).values.tolist()
...
class Hyphen_Deletion_Label(Model):
    """
    --> Delete hyphens
    -->
    """
